...
def wrapper_mean_sst(data, timeframe, bbox = [-999,-999,-999,-999]):
    ...
    try:
        x = delayed(mean_sst)(data, timeframe, bbox)
        x = x.compute()
        return x
...
def mean_sst(data, timeframe, bbox = [-999,-999,-999,-999]):
    ...
    if (start == end):
        ...
        if (bbox != [-999,-999,-999,-999]):
            ds_day = delayed(createSubset)(ds_day, bbox[0], bbox[1], bbox[2], bbox[3])
        ...
        if (bbox != [-999,-999,-999,-999]):
            ds_timeframe = delayed(createSubset)(ds_timeframe, bbox[0], bbox[1], bbox[2], bbox[3])
            ds_timeframe = ds_timeframe.compute()
